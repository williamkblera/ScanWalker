...
class EanBarCode:
   ...
   def getHtml(self, value, height = 50):
      """ Build an HTML bar code """
      result = ""
      ...
      result += '<table><tr><td><center><table cellspacing="0" cellpadding="0" border="0" bgcolor="white"><tr height=%d >'%height
      ...
      for bit in bitsList:
         if bit < 0:
            result += '<td width=%d></td>'%(-bit)
         else:
            result += '<td bgcolor="black" width=%d></td>'%(+bit)
            
      result += '<td width=10></td>'
      result += "</tr></table></center></td></tr><tr><td><center>%s</center></td></tr></table>"%(code)
